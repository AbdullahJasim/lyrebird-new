...
public:
	...
	static int sendMessage(SOCKET currentSocket, std::vector<std::string> lines, int messageSize);

	//Receive function
	static int receiveMessage(SOCKET currentSocket, std::string buffer, int bufferSize);
